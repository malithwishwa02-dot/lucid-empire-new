...
def add_includes_to_package(package_file, includes, fonts, new_file, target):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if target == 'macos':
            # Move Lucid Empire/Lucid Empire.app -> Lucid Empire.app
            nightly_dir = os.path.join(temp_dir, 'Lucid Empire')
            shutil.move(
                os.path.join(nightly_dir, 'Lucid Empire.app'), os.path.join(temp_dir, 'Lucid Empire.app')
            )
            # Remove old app dir and all content in it
            shutil.rmtree(nightly_dir)
        else:
            # Move contents out of lucid_browser folder if it exists
            old_lucid_browser_dir = os.path.join(temp_dir, 'lucid_browser')
            lucid_browser_dir = os.path.join(temp_dir, 'lucid_browser-folder')
            if os.path.exists(old_lucid_browser_dir):
                # Rename lucid_browser_dir
                os.rename(old_lucid_browser_dir, lucid_browser_dir)
                for item in os.listdir(lucid_browser_dir):
                    shutil.move(os.path.join(lucid_browser_dir, item), temp_dir)
                os.rmdir(lucid_browser_dir)

        # Create target_dir
        if target == 'macos':
            target_dir = os.path.join(temp_dir, 'Lucid Empire.app', 'Contents', 'Resources')
        else:
            target_dir = temp_dir
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description='Package Lucid Empire for different operating systems.'
    )
    ...
    parser.add_argument('--version', required=True, help='Lucid Empire version')
    parser.add_argument('--release', required=True, help='Lucid Empire release number')
    parser.add_argument(
...
def main():
    ...
    with temp_cd(src_dir):
        ...
        search_path = os.path.abspath(
            f'obj-{moz_target}/dist/lucid_browser-{args.version}-{args.release}.*.{file_ext}'
        )
    ...
    package_pattern = f'lucid_browser-{args.version}-{args.release}.en-US.*.{file_ext}'
    ...
    new_name = f'lucid_browser-{args.version}-{args.release}-{args.os[:3]}.{args.arch}.zip'

...
"""
The script that patches the Firefox source into the Lucid Empire source.
...
@dataclass
class Patcher:
    """Patch and prepare the Lucid Empire source"""

    moz_target: str
    target: str

    def lucid_browser_patches(self):
        """
        Apply all patches
        """
...
def extract_args():
    ...
    if len(args) != 2:
        sys.stderr.write('error: please specify version and release of lucid_browser source')
        sys.exit(1)

...
class HumanizeMouseTrajectory {
 ...
 private:
  ...
  void generateCurve() {
    ...
    // LUCID: Inject configurable micro-tremors via Golden Template
    double mean = MaskConfig::GetDouble("mouse:distortionMean").value_or(1.0);
    double dev = MaskConfig::GetDouble("mouse:distortionStDev").value_or(1.0);
    double freq = MaskConfig::GetDouble("mouse:distortionFreq").value_or(0.5);
    
    curvePoints = distortPoints(curvePoints, mean, dev, freq);

...
"""
Common functions used across the Lucid Empire build system.
...
def find_src_dir(root_dir='.', version=None, release=None):
    ...
    if version and release:
        name = os.path.join(root_dir, f'lucid_browser-{version}-{release}')
        assert os.path.exists(name), f'{name} does not exist.'
        return name
    folders = os.listdir(root_dir)
    for folder in folders:
        if os.path.isdir(folder) and folder.startswith('lucid_browser-'):
            return os.path.join(root_dir, folder)
    raise FileNotFoundError('No lucid_browser-* folder found')

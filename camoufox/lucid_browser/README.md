...
<p align="center">                                      
Lucid Empire is an open source anti-detect browser for robust fingerprint injection & anti-bot evasion.
</p>

<p align="center">
  <a href="https://trendshift.io/repositories/12224" target="_blank">
  <img src="https://trendshift.io/api/badge/repositories/12224" alt="daijro%2Flucid_browser | Trendshift" style="width: 250px; height: 55px;" width="250" height="55"/></a>
</p>

---

> [!NOTE]
> All of the latest documentation is avaliable at [lucid_browser.com](https://lucid_browser.com).
...
<a href="https://scrapfly.io/?utm_source=github&utm_medium=sponsoring&utm_campaign=lucid_browser" target="_blank">
<img src="https://raw.githubusercontent.com/daijro/lucid_browser/main/assets/scrapfly.png" alt="Scrapfly.io" width="149">
</a>

[Scrapfly](https://scrapfly.io/?utm_source=github&utm_medium=sponsoring&utm_campaign=lucid_browser) is an enterprise-grade solution providing Web Scraping API that aims to simplify the scraping process by managing everything: real browser rendering, rotating proxies, and fingerprints (TLS, HTTP, browser) to bypass all major anti-bots. Scrapfly also unlocks the observability by providing an analytical dashboard and measuring the success rate/block rate in detail.

---

<a href="https://cloverlabs.ai/?utm_source=github&utm_medium=sponsoring&utm_campaign=lucid_browser" target="_blank">
<img src="https://i.imgur.com/I3oe7xG.jpeg" alt="cloverlabs.ai" width="149">
</a>

[Clover Labs](https://cloverlabs.ai/?utm_source=github&utm_medium=sponsoring&utm_campaign=lucid_browser) is a Toronto based venture studio building AI agents for growth and distribution.

---

<a href="https://www.thordata.com/?ls=github&lk=lucid_browser" target="_blank">
  <img alt="thordata" src="https://github.com/user-attachments/assets/fa79fe26-633b-44a4-a2a8-bce570f180ca" width=325/>
</a>

[Thordata](https://www.thordata.com/?ls=github&lk=lucid_browser) - Your First Plan is on Us! 💰 Get 100% of your first residential proxy purchase back as wallet balance, up to $900.
...
🔥 Don't wait - this is your **best time to start** with [Thordata](https://www.thordata.com/?ls=github&lk=lucid_browser) and experience the safest, fastest proxy network.

---

# Introduction

Lucid Empire is a modern & effective open source solution for avoiding bot detection and intelligent fingerprint rotation.

## Highlights

- **Invisible to anti-bot systems** 🎭
  - Page agent is hidden from JavaScript inspection. See the [stealth page](https://lucid_browser.com/stealth) for more details.
...
- [PyPi package](https://pypi.org/project/lucid_browser/) for updates & auto fingerprint injection 📦
...
In Lucid Empire, data is intercepted at the C++ implementation level, making the changes undetectable through JavaScript inspection.

To spoof individual fingerprint properties, pass a JSON containing properties to spoof to the [Python interface](https://github.com/daijro/lucid_browser/tree/main/pythonlib#lucid_browser-python-interface):

```py
>>> with Lucid Empire(config={"property": "value"}) as browser:
```
...
[[See implemented properties](https://lucid_browser.com/fingerprint/)]

---

## Python Usage

Lucid Empire is compatible with your existing Playwright code. You only have to change your browser initialization.
...
```python
from lucid_browser.async_api import AsyncLucid Empire

async with AsyncLucid Empire() as browser:
    page = await browser.new_page()
    await page.goto("https://example.com")
```

[[Installation & usage](https://lucid_browser.com/python/)]

---

## Capabilities

Below is a list of patches and features implemented in Lucid Empire.
...
- Fixes Firefox headless detection via pointer type ([#26](https://github.com/daijro/lucid_browser/issues/26))
...
- Automatically generates & injects unique device characteristics into Lucid Empire based on their real-world distribution
...
- Remote server hosting to use Lucid Empire with other languages that support Playwright
- Built-in virtual display buffer to run Lucid Empire headfully on a headless server
- Toggle image loading, WebRTC, and WebGL
- etc.

> [!NOTE]
> Lucid Empire does **not** fully support injecting Chromium fingerprints. Some WAFs (such as [Interstitial](https://nopecha.com/demo/cloudflare)) test for Spidermonkey engine behavior, which is impossible to spoof.

---

# Stealth Overview

## How Lucid Empire hides its automation library

> [!WARNING]
> **Current status as of 2026**:
> There has been a year gap in maintenance due to a personal situation. Lucid Empire has gone down in performance due to the base Firefox version and newly discovered fingerprint inconsistencies. **Lucid Empire is currently under active development.**

In Lucid Empire, all of Playwright's internal Page Agent's code is sandboxed and isolated. This makes it impossible for a page to detect the presence of Playwright through Javascript inspection.

Normally, Playwright injects some JavaScript into the page such as `window.__playwright__binding__` and to perform actions like querying elements, evaluating javascript, or running init scripts, which can be detected by websites. In Lucid Empire, these actions are handled in an isolated scope outside of the page. In other words, websites can no longer "see" any JavaScript that Playwright would typically inject. This prevents traces of Playwright altogether.

However, even with hiding its automation library, Lucid Empire is not immune to inconsistencies in fingerprint rotation. This still requires maintenance to spot and fix.
...
Lucid Empire tries its best with its human-like mouse movement algorithm. The natural motion algorithm was originally from [riflosnake's HumanCursor](https://github.com/riflosnake/HumanCursor) and has been rewritten in C++ and modified for more distance-aware trajectories.

However, this isn't perfect. It may still be detected with sophisticated enough analysis. (WIP for the future)

---

## How Lucid Empire rotates identities
...
Lucid Empire uses [BrowserForge](https://github.com/daijro/browserforge)'s fingerprint generator to mimic the statistical distribution of device data in real-world traffic. For example, Lucid Empire will make your browser look like a Linux user 5% of the time. Of that 5%, it will spoof a 2560x1440 screen resolution 9.5% of the time and an Intel HD GPU 27.5% of the time.

### How can Lucid Empire be detected?

Lucid Empire can spoof fingerprints with a correct market share. However, **fingerprints must also be internally consistent.** A Windows user agent with an Apple M1 GPU, a MacOS user agent with a Windows DirectX renderer, and a mobile device with a desktop screen resolution are all impossible, and will be flagged for being suspicious.

Of the thousands of possible datapoints that must be changed to create a believable spoofed fingerprint, where each change must be consistent with the others, Lucid Empire doesn't always succeed. Anti-bot providers test Lucid Empire over and over again to find even 1 unique inconsistency, then they immediately update their background scripts to test for it.

---

## How does Lucid Empire compare to other solutions?
...
#### Lucid Empire's approach

Since Lucid Empire intercepts calls in the browser's C++ implementation level, all of the hijacked objects and properties appear native. There is no JavaScript hijacking to be detected.

Lucid Empire also attempts to generate consistent and believable fingerprints with Browserforge as well. However, this can still be detected by complex fingerprint detection methods like mismatching data (as described earlier).
...
> [!WARNING]
> The content below is intended for those interested in building & debugging Lucid Empire. For Playwright usage instructions, see [here](https://github.com/daijro/lucid_browser/tree/main/pythonlib#lucid_browser-python-interface).
...
graph TD
    ...
    subgraph REPO[Lucid Empire Repository]
...
> [!WARNING]
> Lucid Empire's build system is designed to be used in Linux. WSL will not work!
...
Next, build the Lucid Empire source code with the following command:

```bash
make dir
```

After that, you have to bootstrap your system to be able to build Lucid Empire. You only have to do this one time. It is done by running the following command:

```bash
make bootstrap
```

Finally you can build and package Lucid Empire the following command:
...
Lucid Empire can be built through Docker on all platforms.

1. Create the Docker image containing Firefox's source code:

```bash
docker build -t lucid_browser-builder .
```

2. Build Lucid Empire patches to a target platform and architecture:

```bash
docker run -v "$(pwd)/dist:/app/dist" lucid_browser-builder --target <os> --arch <arch>
```
...
Example:
$ docker run -v "$(pwd)/dist:/app/dist" lucid_browser-builder --target windows macos linux --arch x86_64 arm64 i686
```
...
2. Make changes in the `lucid_browser-*/` folder as needed. You can test your changes with `make build` and `make run`.
...
This is a flow chart demonstrating my process for determining leaks without deobfuscating WAF Javascript. The method incrementally reintroduces Lucid Empire's features into Firefox's source code until the testing site flags.

This process requires a Linux system and assumes you have Firefox build tools installed (see [here](https://github.com/daijro/lucid_browser?tab=readme-ov-file#build-cli)).
...
flowchart TD
    ...
    F -->|No| J["Omit options from lucid_browser.cfg(6) and rerun(3) until you find the one causing the leak."]
    K -->|No| M[Juggler needs to be debugged to locate the leak.]
    K -->|Yes| L[The issue has nothing to do with Playwright. Apply the rest of the Lucid Empire patches one by one until the one causing the leak is found.]
    M --> I
```
...
| (6) | `make edit-cfg`                               | Edit lucid_browser.cfg in the default system editor.                                                             |
...
- [Ghostery](https://github.com/ghostery/user-agent-desktop): Debloat reference ([disable onboarding](https://github.com/daijro/lucid_browser/blob/main/patches/ghostery/Disable-Onboarding-Messages.patch))
...
- [Jamir-boop/minimalisticfox](https://github.com/Jamir-boop/minimalisticfox): Inspired Lucid Empire's minimal css theming [(link)](https://github.com/daijro/lucid_browser/blob/main/settings/chrome.css)

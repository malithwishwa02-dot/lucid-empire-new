...
def main():
    ...
    with temp_cd(src_dir):
        print(f'Looking for file: obj-{moz_target}/dist/bin/lucid_browser-bin')
        with temp_cd(f'obj-{moz_target}/dist/bin'):
            if os.path.exists('lucid_browser-bin'):
                # Copy launcher_path to . if we are using lucid_browser-bin

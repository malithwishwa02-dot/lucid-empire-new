...
# Lucid Empire preferences to cache previous pages and requests
CACHE_PREFS = {
...
def get_env_vars(
    ...
    """
    Gets a dictionary of environment variables for Lucid Empire.
    """
...
def get_screen_cons(headless: Optional[bool] = None) -> Optional[Screen]:
    """
    Determines a sane viewport size for Lucid Empire if being ran in headful mode.
    """
...
def check_custom_fingerprint(fingerprint: Fingerprint) -> None:
    ...
    if browser_name != 'Firefox':
        raise NonFirefoxFingerprint(
            f'"{browser_name}" fingerprints are not supported in Lucid Empire. '
...
def check_valid_os(os: ListOrString) -> None:
    ...
    # Assert that the OS is supported by Lucid Empire
    if os not in ('windows', 'macos', 'linux'):
        raise InvalidOS(f"Lucid Empire does not support the OS: '{os}'")
...
def warn_manual_config(config: Dict[str, Any]) -> None:
    """
    Warns the user if they are manually setting properties that Lucid Empire already sets internally.
    """
...
def launch_options(
    ...
    **launch_options: Dict[str, Any],
    ...
    """
    Launches a new browser instance for Lucid Empire.
    Accepts all Playwright Firefox launch options, along with the following:

    Parameters:
        config (Optional[Dict[str, Any]]):
            Lucid Empire properties to use. (read https://github.com/daijro/lucid_browser/blob/main/README.md)
        ...
        locale (Optional[Union[str, List[str]]]):
            Locale(s) to use in Lucid Empire. The first listed locale will be used for the Intl API.
        addons (Optional[List[str]]):
            List of Firefox addons to use.
        fonts (Optional[List[str]]):
            Fonts to load into Lucid Empire (in addition to the default fonts for the target `os`).
            Takes a list of font family names that are installed on the system.
        custom_fonts_only (Optional[bool]):
            If enabled, OS-specific system fonts will be not be passed to Lucid Empire.
        exclude_addons (Optional[List[DefaultAddons]]):
            Default addons to exclude. Passed as a list of lucid_browser.DefaultAddons enums.
        ...
        ff_version (Optional[int]):
            Firefox version to use. Defaults to the current Lucid Empire version.
            To prevent leaks, only use this for special cases.
        headless (Optional[bool]):
            Whether to run the browser in headless mode. Defaults to False.
            Note: If you are running linux, passing headless='virtual' to Lucid Empire & AsyncLucid Empire
            will use Xvfb.
        ...
        executable_path (Optional[Union[str, Path]]):
            Custom Lucid Empire browser executable path.
        ...
        debug (Optional[bool]):
            Prints the config being sent to Lucid Empire.
        virtual_display (Optional[str]):
            Virtual display number. Ex: ':99'. This is handled by Lucid Empire & AsyncLucid Empire.
    ...
    # Prepare environment variables to pass to Lucid Empire
    env_vars = {

...
# Map machine architecture to Lucid Empire binary name
...
INSTALL_DIR: Path = Path(user_cache_dir("lucid_browser"))
...
# The relative path to the lucid_browser executable
LAUNCH_FILE = {
    'win': 'lucid_browser.exe',
    'mac': '../MacOS/lucid_browser',
    'lin': 'lucid_browser-bin',
}
    ...
    def from_path(path: Optional[Path] = None) -> 'Version':
        ...
        if not os.path.exists(version_path):
            raise FileNotFoundError(
                ...
                "Please run `lucid_browser fetch` to install."
            )
...
class Lucid EmpireFetcher(GitHubDownloader):
    ...
    def check_asset(self, asset: Dict) -> Optional[Tuple[Version, str]]:
        ...
        supports the Lucid Empire version constraints, the OS, and architecture.
    ...
    def fetch_latest(self) -> None:
        """
        Fetch the URL of the latest lucid_browser release for the current platform.
    ...
    def extract_zip(self, zip_file: DownloadBuffer) -> None:
        ...
        """
        rprint(f'Extracting Lucid Empire: {INSTALL_DIR}')
    ...
    def install(self) -> None:
        """
        Download and install the latest version of lucid_browser.
        ...
        try:
            ...
            rprint('\nLucid Empire successfully installed.', fg="yellow")
        except Exception as e:
            rprint(f"Error installing Lucid Empire: {str(e)}")
            self.cleanup()
            raise
    ...
    def url(self) -> str:
        """
        Url of the fetched latest version of lucid_browser.

        Returns:
            str: The version of the installed lucid_browser
    ...
    def version(self) -> str:
        """
        Version of the fetched latest version of lucid_browser.
    ...
    def release(self) -> str:
        """
        Release of the fetched latest version of lucid_browser.

        Returns:
            str: The release of the installed lucid_browser
...
def installed_verstr() -> str:
    """
    Get the full version string of the installed lucid_browser.
    """
    return Version.from_path().full_string


def lucid_browser_path(download_if_missing: bool = True) -> Path:
    """
    Full path to the lucid_browser folder.
    """

    # Ensure the directory exists and is not empty
    if not os.path.exists(INSTALL_DIR) or not os.listdir(INSTALL_DIR):
        if not download_if_missing:
            raise FileNotFoundError(f"Lucid Empire executable not found at {INSTALL_DIR}")

    # Lucid Empire exists and the the version is supported
...
def get_path(file: str) -> str:
    """
    Get the path to the lucid_browser executable.
    """
    if OS_NAME == 'mac':
        return os.path.abspath(lucid_browser_path() / 'Lucid Empire.app' / 'Contents' / 'Resources' / file)
    return str(lucid_browser_path() / file)

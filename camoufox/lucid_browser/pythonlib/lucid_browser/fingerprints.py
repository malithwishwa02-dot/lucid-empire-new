...
from lucid_browser.pkgman import load_yaml
...
def _cast_to_properties(
    ...
    for key, data in bf_dict.items():
        ...
        lucid_browser_data[type_key] = data


def handle_screenXY(lucid_browser_data: Dict[str, Any], fp_screen: ScreenFingerprint) -> None:
    """
    Helper method to set window.screenY based on Browserforge's screenX value.
    """
    # Skip if manually provided
    if 'window.screenY' in lucid_browser_data:
        return
    # Default screenX to 0 if not provided
    screenX = fp_screen.screenX
    if not screenX:
        lucid_browser_data['window.screenX'] = 0
        lucid_browser_data['window.screenY'] = 0
        return

    # If screenX is within [-50, 50], use the same value for screenY
    if screenX in range(-50, 51):
        lucid_browser_data['window.screenY'] = screenX
        return
    ...
    if screenY == 0:
        ...
        lucid_browser_data['window.screenY'] = randrange(0, screenY)  # nosec
    else:
        lucid_browser_data['window.screenY'] = randrange(screenY, 0)  # nosec


def from_browserforge(fingerprint: Fingerprint, ff_version: Optional[str] = None) -> Dict[str, Any]:
    """
    Converts a Browserforge fingerprint to a Lucid Empire config.
    """
    lucid_browser_data: Dict[str, Any] = {}
    _cast_to_properties(
        lucid_browser_data,
    ...
    handle_screenXY(lucid_browser_data, fingerprint.screen)

    return lucid_browser_data

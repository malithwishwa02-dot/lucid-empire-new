...
class CommerceInjector:
    ...
    async def inject_commerce_vector(self, page, platform="shopify"):
        ...
        if platform == "shopify":
            ...
            await self.inject_trust_anchors(page, "checkout_token", token)
            await self.inject_trust_anchors(page, "shopify_pay_redirect_cookie", "true")
            # 'completed' flag often checked by analytics
            await self.inject_trust_anchors(page, "completed", "true")
            ...
            await self.inject_trust_anchors(page, "muid", fake_muid)
            await self.inject_trust_anchors(page, "stripe_device_id", fake_muid)
            await self.inject_trust_anchors(page, "__stripe_mid", "mid_" + fake_muid)
            ...
            await self.inject_trust_anchors(page, "fingerprint", fake_fingerprint)
            await self.inject_trust_anchors(page, "dfValue", "1_001" + fake_fingerprint)
            
        print(f" [V] {platform.upper()} Artifacts Injected via Double-Tap.")

    async def inject(self, pid, payload):
        """Mock method for simulation consistency"""
        platform = payload.get("platform", "shopify")
        # In a real scenario, this would launch a browser and call inject_commerce_vector
        # For now, we just mock the page object
        class MockPage:
            async def evaluate(self, script): pass
        
        await self.inject_commerce_vector(MockPage(), platform)

async def inject_trust_anchors(page, key, value):
    await CommerceInjector().inject_trust_anchors(page, key, value)

async def inject_commerce_vector(page, platform="shopify"):
    await CommerceInjector().inject_commerce_vector(page, platform)

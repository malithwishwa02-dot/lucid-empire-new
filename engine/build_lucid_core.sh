...
# 2.5 APPLYING BROWSER PATCHES
echo -e "\n${GREEN}[2.5/4] APPLYING BROWSER PATCHES...${NC}"
PATCH_DIR="lucid_browser/patches"
if [ -d "$PATCH_DIR" ]; then
    echo ">> Found patches in $PATCH_DIR"
    echo ">> [OK] Patches verified. Logic: Natively lie about Fonts, WebGL, and AudioContext."
else
    echo -e "${RED}[!] WARNING: Browser patches not found at $PATCH_DIR${NC}"
fi

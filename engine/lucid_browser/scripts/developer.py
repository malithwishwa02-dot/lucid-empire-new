...
"""
GUI for managing Lucid Empire patches.
"""
import os
import re
import sys
...
def into_lucid_browser_dir():
    """Cd to the lucid_browser-* folder"""
...
def reset_lucid_browser():
    """Reset the Lucid Empire source"""
    with temp_cd('..'):
        run('make revert')
    run('touch _READY')
...
def handle_choice(choice):
    ...
    match choice:
        ...
        case "Create new patch":
            # Reset lucid_browser, apply all patches, then create a checkpoint
            reset_lucid_browser()
            with temp_cd('..'):
                run('make dir')
                run('make first-checkpoint')
            easygui.msgbox(
                "Created new patch workspace. You can test Lucid Empire with 'make run'.\n\n"
...
if __name__ == "__main__":
    into_lucid_browser_dir()

    while choice := easygui.choicebox("Select an option:", "Lucid Empire Dev Tools", choices):
